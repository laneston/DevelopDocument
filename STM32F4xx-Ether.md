...
有外部PHY接口并支持10/100mbit/s的数据传输速率。

符合IEEE 802.3标准的MII接口，用于与外部快速以太网PHY通信。

支持全双工和半双工操作：

l 支持用于半双工操作的CSMA/CD协议；

l 支持全双工操作的IEEE 802.3x流量控制；

l 在全双工操作中，可选地将接收到的暂停控制帧转发给用户应用程序；

l 背压支持半双工操作；

l 在全双工操作中，流量控制输入解除时，自动发送零量程暂停帧。

在发送中插入前导码和帧起始数据（SFD），在接收路径中删除。

在每帧的基础上实行自动CRC校验和生成PAD。（以太网帧长度最小64字节，长度不够的需要加PAD。）

接收帧上的自动PAD/CRC剥离选项。

可编程的帧长度，支持最大为16kb的标准帧。

可编程帧间隙（40-96bit 时长）。

支持多种灵活的地址筛选模式：

l 最多4个48位完美（DA）地址过滤器，每个字节都有掩码；

l 最多3个48位SA地址比较检查，每个字节都有掩码；

l 用于多播和单播（DA）地址的64位哈希过滤器（可选）；

l 通过所有多播寻址帧的选项；

l 支持混杂模式，无需任何过滤即可通过所有帧进行网络监控；

l 使用状态报告传递所有传入数据包（根据筛选器）。

为发送和接收数据包返回单独的状态位（32bit）。

支持接收帧的IEEE 802.1Q VLAN标记检测。

独立的传输和接收以及应用控制接口。

支持带有RMON/MIB计数器（RFC2819/RFC2665）的强制网络统计信息。

用于PHY设备配置和管理的MDIO接口。

LAN唤醒帧和AMD Magic Packet帧检测。

以太网帧封装的已接收IPv4和TCP数据包的校验和卸载接收功能。

增强的接收功能，用于检查IPv4头校验和以及封装在IPv4或IPv6数据报中的TCP、UDP或ICMP的校验和。

支持IEEE1588-2008中描述的以太网帧时间戳。在每帧的发送或接收状态下给出64位时间戳。

两组FIFO：一个具有可编程阈值能力的2-KB传输FIFO，一个具有可配置阈值的2-KB接收FIFO（默认为64字节）

在EOF传输后插入接收FIFO的接收状态向量允许在接收FIFO中存储多个帧，而不需要另一个FIFO来存储这些帧的接收状态。

选项在接收时过滤所有错误帧，而不在存储和转发模式下将其转发到应用程序。

选择转发尺寸不足的好帧。

通过为接收队列中丢失或损坏的帧（由于溢出）生成脉冲来支持统计。

支持向MAC核传输的存储转发机制。

基于接收队列填充（阈值可配置）水平，自动生成暂停帧控制或返回到MAC核心的压力信号。

处理碰撞帧的自动重新传输以进行传输。

在后期碰撞、过度碰撞、过度延迟和欠载情况下丢弃帧。

刷新Tx 队列的软件控制。

存储和转发模式下，在传输的帧中计算并插入IPv4头校验和和和TCP、UDP或ICMP的校验和。
...
描述符架构，允许大数据块传输，CPU干预最少。

每个描述符最多可以传输8 KB的数据。

全面的正常运行和错误传输状态报告。

用于发送和接收DMA引擎的单个可编程突发大小，以实现最佳主机总线利用率。
