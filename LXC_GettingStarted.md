...
容器是系统内部的隔离区域，它们有自己的文件系统、网络、PID、IPC、CPU 和内存分配的名称空间，并且可以使用 Linux 内核中包含的控制组(cgroup)和名称空间(namespace)功能来创建。这提供了一个可插入的身份验证模块（PAM），为登录用户提供一组 cgroup，供他们管理。例如，这允许使用非特权容器和使用 cgroup 进程跟踪进行会话管理。
