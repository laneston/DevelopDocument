...
当一个工程项目十分庞大，所涉及的文件数目也会因此递增，并且伴随着频繁的文件增减，Makefile文件也需要跟着频繁修改，这会令写 Makefile 变得十分困难，而且容易出错。有没有什么办法可以写一个“万能”的 Makefile 文件，从而一劳永逸呢？答案是可以的！在一个工程模板不变的情况下，是可以做到编写一个通用的 Makefile 的。