...
**发送命令到 NAND Flash memory：** 程序必须写命令值到命令部分的任何位置。

**读或写指定 NAND Flash 地址：** 程序必须写地址值到地址部分的任务位置。地址可以为4或5个字节长度，这取决于实际内存大小，要指定完整地址，需要对地址部分进行多次连续写入。

**读或写数据：** 程序可以在数据部分的任何内存位置读或写数据值。
